...
Description: PETCGDNN训练文件 - 基于main_MCNet.py风格
- 支持分布式训练和混合精度训练
- 适配PETCGDNN模型的数据格式
- 使用Focal Loss提升训练效果
- 支持断点续训和最佳模型保存
...
'''
from model_compare.PETCGDNN import PETCGDNN as Net
import time
...
def setup(rank, world_size):
    ...
    os.environ['MASTER_PORT'] = '12347'  # 确保此端口没有被占用
...
def train(rank, world_size, num_gpus, num_epochs, continue_train=False):
    ...
    if rank==0:
        # current_time = time.strftime("%Y%m%d-%H%M%S", time.localtime())
        tb_writer = SummaryWriter(os.path.join(PATH, current_time+'PETCGDNN'))
        tb_dir = os.path.join(PATH, current_time+'PETCGDNN')
        ...
        # 保存主要的Python文件
        try:
            shutil.copy2('main_PETCGDNN.py', os.path.join(tb_dir, 'main_PETCGDNN.py'))
            print(f"Saved main_PETCGDNN.py to {tb_dir}")
        except Exception as e:
            print(f"Failed to save main_PETCGDNN.py: {e}")
            
        # 保存网络模型文件
        try:
            shutil.copy2('model_compare/PETCGDNN.py', os.path.join(tb_dir, 'PETCGDNN.py'))
            print(f"Saved PETCGDNN.py to {tb_dir}")
        except Exception as e:
            print(f"Failed to save PETCGDNN.py: {e}")
    ...
    # PETCGDNN模型实例化，输入格式: (batch_size, seq_len=1024, features=2)
    # input_shape参数格式: [seq_len, features]
    model = Net(input_shape=[1024, 2], classes=15)  # 15个调制类型
    ...
    base_batch_size = 128  # PETCGDNN相对轻量，可以使用更大的batch_size
    batch_size_multiplier = 1  # 可以根据需要调整
    base_lr = 0.001
    
    # 根据模型大小动态调整batch_size和学习率
    if model_params < 100000:  # 小网络（PETCGDNN属于这类）
        base_batch_size = 256
        batch_size_multiplier = 1
        base_lr = 0.001
        weight_decay = 1e-4
        print(f"Rank {rank}: Small model detected, using optimized settings")
    elif model_params < 500000:  # 中等网络
        base_batch_size = 128
        batch_size_multiplier = 1
        base_lr = 0.0005
        weight_decay = 5e-4
    ...
    # 优化数据预处理 - PETCGDNN使用I/Q数据格式
    def prepare_data(dataset):
        data = dataset.tensors[0]  # 已经是 (batch, seq_len, features) 格式
        labels = dataset.tensors[1]
        # PETCGDNN期望输入格式为 (batch, seq_len, features)，原始数据已经是这个格式
...
def main():
    # 添加命令行参数解析
    parser = argparse.ArgumentParser(description='PETCGDNN RadioML Classification Training')
    ...
    print(f"Starting PETCGDNN training with {num_epochs} epochs")
