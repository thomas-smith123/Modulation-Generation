...
from model_compare.ICAMC import ICAMC as Net
import time
...
def train(rank, world_size, num_gpus, num_epochs, continue_train=False):
    ...
    if rank==0:
        # current_time = time.strftime("%Y%m%d-%H%M%S", time.localtime())
        tb_writer = SummaryWriter(os.path.join(PATH, current_time+'ICAMC'))
        tb_dir = os.path.join(PATH, current_time+'ICAMC')
        ...
        # 保存主要的Python文件
        try:
            shutil.copy2('main_ICAMC.py', os.path.join(tb_dir, 'main_ICAMC.py'))
            print(f"Saved main.py to {tb_dir}")
        except Exception as e:
            print(f"Failed to save main_ICAMC.py: {e}")
            
        # 保存网络模型文件
        try:
            shutil.copy2('model_compare/ICAMC.py', os.path.join(tb_dir, 'ICAMC.py'))
            print(f"Saved modelv0_7.py to {tb_dir}")
        except Exception as e:
            print(f"Failed to save ICAMC.py: {e}")
    ...
    model = Net(input_shape=[2, 1024], modulation_num=15)  # 15个调制类型
