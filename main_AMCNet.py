...
from model_compare.AMCNet import AMCNet as Net
import time
...
def train(rank, world_size, num_gpus, num_epochs, continue_train=False):
    ...
    if rank==0:
        # current_time = time.strftime("%Y%m%d-%H%M%S", time.localtime())
        tb_writer = SummaryWriter(os.path.join(PATH, current_time+'AMCNet'))
        tb_dir = os.path.join(PATH, current_time+'AMCNet')
        ...
        # 保存主要的Python文件
        try:
            shutil.copy2('main_AMCNet.py', os.path.join(tb_dir, 'main_AMCNet.py'))
            print(f"Saved main.py to {tb_dir}")
        except Exception as e:
            print(f"Failed to save main_AMCNet.py: {e}")
            
        # 保存网络模型文件
        try:
            shutil.copy2('model_compare/AMCNet.py', os.path.join(tb_dir, 'AMCNet.py'))
            print(f"Saved modelv0_7.py to {tb_dir}")
        except Exception as e:
            print(f"Failed to save AMCNet.py: {e}")
    ...
    model = Net(input_shape=[2, 1024], modulation_num=15)  # 修复：input_shape应该是[2, 1024]
    ...
    base_lr = 0.002
    # 轻量级模型可以使用更大的batch size和学习率
    if model_params < 50000:  # 现在应该在这个范围
        base_batch_size = 256  # 增加batch size
        batch_size_multiplier = 1
        base_lr = 0.002  # 适中的学习率
        weight_decay = 1e-3  # 适度的权重衰减
        print(f"Rank {rank}: Lightweight model detected, using optimized settings")
    elif model_params < 100000:
        base_batch_size = 128
        batch_size_multiplier = 1
        base_lr = 0.001
        weight_decay = 5e-4
        print(f"Rank {rank}: Small model detected")
    else:
        base_batch_size = 64
        batch_size_multiplier = 1
        base_lr = 0.0005
