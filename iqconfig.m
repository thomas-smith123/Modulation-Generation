...
'M8199B',          '224GSa/s option',                       0, {'amplitude','offset','moduleID','multiModule','skew','peaking','ampType','sampleMarker'}; ...
...
'M8198A',          [],                                      0, {'amplitude','offset','moduleID','skew','ampType', 'sampleMarker'}; ...
...
%try
    ...
    if (strcmp(arbConfig.model, 'M8198A'))
        set(handles.editTimeout, 'String', '120');
    end
...
if contains(arbConfig.model, 'M8198A')
    set(handles.popupmenuSampleMarker, 'TooltipString', sprintf([ ...
    'Recommended connections for M8198A + DCA:\n' ...
    'Option 1 (preferred): \n' ...
    'M8008A RefClkOut16G -> power divider -> Trigger + PTB \n' ...
    'Use Sample Marker / 8 \n' ...
    'or \n' ...
    'Option 2: \n '....
    'Sample Marker Out 1 -> Trigger + PTB \n' ...
    'Use Sample Marker / 16']));
end
end
...
awgList = get(handles.popupmenuModel, 'String');
awgModel = awgList(get(handles.popupmenuModel, 'Value'));
if contains(awgModel{1}, 'M8198A')
    markerList = get(handles.popupmenuSampleMarker, 'String');
    selectedMarker = markerList(get(handles.popupmenuSampleMarker, 'Value'));
    
    if ~contains(selectedMarker{1}, ' / 8') && ~contains(selectedMarker{1}, ' / 16') && ~contains(selectedMarker{1}, ' / 32') && ~contains(selectedMarker{1}, ' / 64') && ~contains(selectedMarker{1}, ' unchanged')
        set(handles.popupmenuSampleMarker, 'Value', 6);
        set(handles.popupmenuSampleMarker, 'Background', 'yellow');
    end
    drawnow();
    pause(0.5);
