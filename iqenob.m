...
if (strcmpi(analyzer, 'dca_ieee') || strcmpi(analyzer, 'rts'))
    if contains(arbConfig.model, 'M8199A') || contains(arbConfig.model, 'M8199B') || contains(arbConfig.model, 'M8199A') 
        enobs = iqenobieee_M8199A_ILV(analyzer, arbConfig, tones, fsAWG, ...
...
if (awgTrig == 5)  % trigger using markers on M8190A & M8195A_1ch
    ...
    elseif (~isempty(strfind(arbConfig.model, 'M8195A_1ch')))
        ...
        xfprintf(f, sprintf(':VOLT4:AMPL %g; offs %g', trigAmpl, trigOffs));
    else
...
%     end
    if (~isempty(hMsgBox))
        hMsgBox.update(0.02, 'Trying to connect to DCA...');
    end
...
%     end
    if (~isempty(hMsgBox))
        hMsgBox.update(0.02, 'Trying to connect to spectrum analyzer...');
    end
...
%         waitbar((2*i-1)/(2*numTones), hMsgBox, 'Downloading waveform to AWG...');
        hMsgBox.update((2*i-1)/(2*numTones), 'Downloading waveform to AWG...');
    end
...
%         waitbar((2*i)/(2*numTones), hMsgBox, 'Reading data from analyzer...');
        hMsgBox.update((2*i)/(2*numTones), 'Reading data from analyzer...');
    end
...
toneFreq = str2double(xquery(f, 'CALC:MARK1:X?'));
...
n1dB = str2double(xquery(f, 'CALC:MARK2:Y?'));
n1Pwr = 10^(n1dB/10);
if (fmin >= toneLeft)
    n1Pwr = 0;
end
n2dB = str2double(xquery(f, 'CALC:MARK3:Y?'));
n2Pwr = 10^(n2dB/10);
if (toneRight >= fmax)
    n2Pwr = 0;
end
if (toneNum == 1)
    sigDb = str2double(xquery(f, 'CALC:MARK1:Y?'));
    sigPwr = 10^(sigDb/10);
end
if (sigDb < -60)
