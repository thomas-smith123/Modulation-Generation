...
def img2label_paths(img_paths):
    ...
    if 'npy' in list(img_paths)[0]:
        sa, sb = os.sep + 'stft_complex_' + os.sep, os.sep + 'labels' + os.sep  # /images/, /labels/ substrings
        ...
        if mosaic:
            ...
            if random.random() < hyp['mixup']:
                ...
                if img.dtype == np.complex64 or img.dtype == np.complex128:
                    img = (img * r + img2 * (1 - r)).astype(np.complex64)
                else:    
...
def load_mosaic(self, index):
    ...
    for i, index in enumerate(indices):
        ...
        if i == 0:  # top left
            if img.dtype == np.complex64 or img.dtype == np.complex128:
                img4 = np.full((s * 2, s * 2, img.shape[2]), 0, dtype=np.complex64)
            else:
                # img4 = np.full((s * 2, s * 2, img.shape[2]), 114, dtype=np.uint8)  # base image with 4 tiles
                img4 = np.full((s * 2, s * 2, img.shape[2]), 0, dtype=np.complex64)  # base image with 4 tiles
...
def load_mosaic9(self, index):
    ...
    for i, index in enumerate(indices):
        ...
        if i == 0:  # center
            if img.dtype == np.complex64 or img.dtype == np.complex128:
                img9 = np.full((s * 3, s * 3, img.shape[2]), 0, dtype=np.complex64)
            else:
                # img9 = np.full((s * 3, s * 3, img.shape[2]), 114, dtype=np.uint8)  # base image with 4 tiles
                img9 = np.full((s * 3, s * 3, img.shape[2]), 0, dtype=np.complex64)  # base image with 4 tiles
            h0, w0 = h, w
    ...
    for i, index in enumerate(indices):
        ...
        if i == 0:  # top left
            if img.dtype == np.complex64 or img.dtype == np.complex128:
                ...
                # img4 = np.full((s * 2, s * 2, img.shape[2]), 114, dtype=np.uint8)  # base image with 4 tiles
                img4 = np.full((s * 2, s * 2, img.shape[2]), 114, dtype=np.complex64)  # base image with 4 tiles
            x1a, y1a, x2a, y2a = max(xc - w, 0), max(yc - h, 0), xc, yc  # xmin, ymin, xmax, ymax (large image)
            x1b, y1b, x2b, y2b = w - (x2a - x1a), h - (y2a - y1a), w, h  # xmin, ymin, xmax, ymax (small image)
        elif i == 1:  # top right
            x1a, y1a, x2a, y2a = xc, max(yc - h, 0), min(xc + w, s * 2), yc
...
def random_perspective(img, targets=(), segments=(), degrees=10, translate=.1, scale=.1, shear=10, perspective=0.0,
    ...
    if (border[0] != 0) or (border[1] != 0) or (M != np.eye(3)).any():  # image changed
        if perspective:
            ...
            if img.dtype == np.complex64 or img.dtype == np.complex128:
                img_r = cv2.warpAffine(img.real, M[:2], dsize=(width, height), borderValue=(0, 0, 0))
                img_i = cv2.warpAffine(img.imag, M[:2], dsize=(width, height), borderValue=(0, 0, 0))
                img = img_r + 1j * img_i
                img = np.expand_dims(img, -1)
            else:
    ...
    if n:
        ...
        # filter candidates
        i = box_candidates(box1=targets[:, 1:5].T * s, box2=new.T, area_thr=0.005 if use_segments else 0.10)
        targets = targets[i]
        targets[:, 1:5] = new[i]
...
def box_candidates(box1, box2, wh_thr=2, ar_thr=45, area_thr=0.1, eps=1e-16):  # box1(4,n), box2(4,n)
