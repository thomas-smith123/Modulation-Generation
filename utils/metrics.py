...
def ap_per_class(tp, conf, pred_cls, target_cls, v5_metric=False, plot=False, save_dir='.', names=()):
    ...
    df = pd.DataFrame(p)
    df.to_csv(Path(save_dir) / 'P_curve.csv',index=False,header=False)
    df = pd.DataFrame(r)
    df.to_csv(Path(save_dir) / 'r_curve.csv',index=False,header=False)
    df = pd.DataFrame(f1)
    df.to_csv(Path(save_dir) / 'F1_curve.csv',index=False,header=False)
