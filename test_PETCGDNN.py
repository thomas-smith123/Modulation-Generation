...
Description: PETCGDNN测试文件 - 基于test_MCNet.py风格
- 支持生成不同SNR下的混淆矩阵和总体混淆矩阵
- 适配PETCGDNN模型的数据格式 (batch, seq_len, features)
- 与main_PETCGDNN.py保持一致的设置和优化
...
'''
# 导入需要的模块
from model_compare.PETCGDNN import PETCGDNN as Net  # 根据main_PETCGDNN.py使用的模型版本
import time
...
# 设置GPU - 与main_PETCGDNN.py保持一致
os.environ["CUDA_VISIBLE_DEVICES"] = "0,7,1,2,3,4,5,6"
# 设置训练结果路径 - 与main_PETCGDNN.py保持一致
BASE_PATH = './runs'
PATH = None

# 自动寻找最新的PETCGDNN训练结果
if os.path.exists(BASE_PATH):
    petcgdnn_dirs = [d for d in os.listdir(BASE_PATH) if os.path.isdir(os.path.join(BASE_PATH, d)) and 'PETCGDNN' in d]
    if petcgdnn_dirs:
        # 按时间排序，选择最新的
        petcgdnn_dirs.sort(reverse=True)
        PATH = os.path.join(BASE_PATH, petcgdnn_dirs[0])
        print(f"Using latest PETCGDNN training results from: {PATH}")
    else:
        print(f"No PETCGDNN training results found in {BASE_PATH}, using default path")
        PATH = BASE_PATH
else:
...
def validate_model(model, val_loader, device, class_num=15, snr_list=None):
    ...
    with torch.no_grad():
        for batch_idx, batch_data in enumerate(val_loader):
            ...
            # PETCGDNN数据预处理：确保数据格式为 (batch, seq_len, features)
            ...
            # 前向传播 - PETCGDNN期望输入格式为 (batch, seq_len, features)
...
def prepare_data_for_petcgdnn(dataset):
    """
    为PETCGDNN准备数据格式
    PETCGDNN期望输入格式为 (batch, seq_len, features)
    """
    # 检查原始数据格式
    ...
    print(f"Final data shape for PETCGDNN: {data_real.shape}")
    print(f"Final data type: {data_real.dtype}")
    
    return data_real, labels


def generate_snr_confusion_matrices(model_path, save_dir="test_results_PETCGDNN", test_batch_size=32):
    """
    ...
    try:
        ...
        test_data, test_labels = prepare_data_for_petcgdnn(test_dataset)
    ...
    try:
        # PETCGDNN模型实例化，与main_PETCGDNN.py保持一致
        model = Net(input_shape=[1024, 2], classes=15)
        
        # 确定搜索路径（优先使用PETCGDNN训练目录）
        search_path = PATH if PATH != BASE_PATH else BASE_PATH
        
        # 优先尝试加载PETCGDNN训练目录中的最佳模型
        ...
        if os.path.exists(best_model_path):
            ...
            print(f"Loaded best model from PETCGDNN training directory: {best_model_path}")
        else:
    ...
    for snr in snr_list:
        ...
        fig = plot_confusion_matrix(cm, 
                                  title=f"PETCGDNN Confusion Matrix (SNR={snr}dB, Acc={accuracy:.3f})",
        ...
        fig_raw = plot_confusion_matrix(cm, 
                                      title=f"PETCGDNN Confusion Matrix Raw Counts (SNR={snr}dB)",
                                      class_names=modulation_classes, 
                                      normalize=False)
        ...
        with open(os.path.join(save_dir, f'metrics_snr_{snr}dB.txt'), 'w') as f:
            f.write(f"PETCGDNN Performance Metrics for SNR = {snr} dB\n")
            f.write("=" * 50 + "\n\n")
    ...
    try:
        ...
        fig_total = plot_confusion_matrix(overall_cm, 
                                        title=f"PETCGDNN Overall Confusion Matrix (Acc={overall_accuracy:.3f})",
                                        class_names=modulation_classes, 
                                        normalize=True)
        ...
        fig_total_raw = plot_confusion_matrix(overall_cm, 
                                            title="PETCGDNN Overall Confusion Matrix (Raw Counts)",
                                            class_names=modulation_classes, 
                                            normalize=False)
        ...
        with open(os.path.join(save_dir, 'overall_metrics.txt'), 'w') as f:
            f.write("PETCGDNN Overall Performance Metrics\n")
            f.write("=" * 40 + "\n\n")
    ...
    if snr_accuracies:
        ...
        plt.xlabel('SNR (dB)')
        plt.ylabel('Accuracy')
        plt.title('PETCGDNN: Accuracy vs SNR')
        ...
        with open(os.path.join(save_dir, 'snr_accuracies.txt'), 'w') as f:
            f.write("PETCGDNN SNR vs Accuracy\n")
            f.write("=" * 25 + "\n")
            for snr in snrs:
...
def main():
    ...
    if best_model_path and os.path.exists(best_model_path):
        ...
        print("Please ensure you have trained a model using main_PETCGDNN.py first")
        print("Available files in search path:")
        if os.path.exists(search_path):
            all_files = os.listdir(search_path)
            print(f"  Total files: {len(all_files)}")
            pth_files = [f for f in all_files if f.endswith('.pth')]
            print(f"  .pth files: {pth_files}")  # 显示所有.pth文件
        return
    ...
    save_dir = f"test_results_PETCGDNN_{timestamp}"
