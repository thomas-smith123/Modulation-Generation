...
# @LastEditTime: 2025-08-18 16:38:49
# @FilePath: /complex_gru_single_radioml/main_gru_only.py
# @Author: thomas-smith123 thomas-smith@live.cn
# @Description: 
# @
# @Copyright (c) 2024 by thomas-smith123 &&, All Rights Reserved. 
...
from model.modelv0_7_gru_only import Net
import time
...
def setup(rank, world_size):
    ...
    os.environ['MASTER_PORT'] = '12344'  # 确保此端口没有被占用
...
def train(rank, world_size, num_gpus, num_epochs, continue_train=False):
    ...
    if rank==0:
        # current_time = time.strftime("%Y%m%d-%H%M%S", time.localtime())
        tb_writer = SummaryWriter(os.path.join(PATH, current_time+'GRU_only'))
        tb_dir = os.path.join(PATH, current_time+'GRU_only')
