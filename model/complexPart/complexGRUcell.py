...
# @LastEditTime: 2024-10-24 10:17:10
# @FilePath: /hy_bak_test_delete_after_used/complex_gru_single_radioml/model/complexPart/complexGRUcell.py
...
try:
    ...
    from model.complexPart.activationComplex import ComplexSigmoid2, tanh_modified, ComplexSigmoid1
    # from model.complexPart.activationModule import complex_sigmoid
    from model.complexPart.convComplex import Conv2dComplex
except:
    from complexPart.linearComplex import LinearComplex
    from complexPart.activationComplex import ComplexSigmoid2, tanh_modified, ComplexSigmoid1
    # from complexPart.activationModule import complex_sigmoid
...
class CustomGRUCell(nn.Module):
    def __init__(self, input_size, hidden_size):
        ...
        self.sigmoid = ComplexSigmoid1()
        self.activation = tanh_modified() ##FIXME 要换的

    def forward(self, x, h):
        tmp_r = torch.concat((x[:x.size(0) // 2, ...], h[:h.size(0) // 2, ...]),1)
        tmp_i = torch.concat((x[x.size(0) // 2:, ...], h[h.size(0) // 2:, ...]),1)
        combined = torch.concat((tmp_r, tmp_i),0)
        # combined = torch.cat((x, h), dim=1) ##FIXME
        z = self.sigmoid(self.Wz(combined))
        r = self.sigmoid(self.Wr(combined))
        tmp = torch.concat((r,r),0)*h
        tmp_r = torch.concat((x[:x.size(0) // 2, ...], tmp[:tmp.size(0) // 2, ...]),1)
        tmp_i = torch.concat((x[x.size(0) // 2:, ...], tmp[tmp.size(0) // 2:, ...]),1)
        combined_r = torch.concat((tmp_r, tmp_i),0)
        # combined_r = torch.cat((x, r * h), dim=1)
        h_tilde = self.activation(self.Wh(combined_r))
        h_next = (1 - torch.concat((z,z),0)) * h + torch.concat((z,z),0) * h_tilde
        return h_next
...
class ConvGRU(nn.Module):
    def __init__(self, input_channels, hidden_channels, kernel_size, num_layers, padding=0):
    ...
    def forward(self, x, hidden_states=None):
        ...
        # features_r = x[:, :C//2, :, :]
        # features_i = x[:, C//2:, :, :]
        # features_r = features_r.permute(0,3,2,1).reshape(B,W,H*(C//2))
        # features_i = features_i.permute(0,3,2,1).reshape(B,W,H*(C//2))
        # x = torch.cat((features_r,features_i),1) ## 和后面的gru对实部和虚部的处理保持一致
...
if __name__ == "__main__":
    # Initialize a ConvGRU
    conv_gru = ConvGRU(input_channels=3, hidden_channels=16, kernel_size=3, num_layers=2)
    
    # Example input (batch_size=4, channels=3, height=64, width=64)
    x = torch.randn(4, 3, 64, 64)
    
    # Forward pass through ConvGRU
    output, hidden = conv_gru(x)
