...
Description: CSGNet测试文件 - 基于test_daelstm.py风格
- 支持生成不同SNR下的混淆矩阵和总体混淆矩阵
- 适配CSGNet模型的数据格式 (batch, seq_len, features)
- 与main_CSGNet.py保持一致的设置和优化
...
'''
# 导入需要的模块
from model_compare.CSGNet import CSGNet as Net  # 根据main_CSGNet.py使用的模型版本
import time
...
# 设置GPU - 与main_CSGNet.py保持一致
os.environ["CUDA_VISIBLE_DEVICES"] = "0,7,1,2,3,4,5,6"
# 设置训练结果路径 - 与main_CSGNet.py保持一致
BASE_PATH = './runs'
PATH = None

# 自动寻找最新的CSGNet训练结果
if os.path.exists(BASE_PATH):
    csgnet_dirs = [d for d in os.listdir(BASE_PATH) if os.path.isdir(os.path.join(BASE_PATH, d)) and 'CSGNet' in d]
    if csgnet_dirs:
        # 按时间排序，选择最新的
        csgnet_dirs.sort(reverse=True)
        PATH = os.path.join(BASE_PATH, csgnet_dirs[0])
        print(f"Using latest CSGNet training results from: {PATH}")
    else:
        print(f"No CSGNet training results found in {BASE_PATH}, using default path")
        PATH = BASE_PATH
else:
...
def validate_model(model, val_loader, device, class_num=15, snr_list=None):
    ...
    with torch.no_grad():
        for batch_idx, batch_data in enumerate(val_loader):
            ...
            # CSGNet数据预处理：确保数据格式为 (batch, seq_len, features)
            ...
            # 前向传播 - CSGNet期望输入格式为 (batch, seq_len, features)
...
def test_model():
    ...
    # 准备验证数据加载器 - CSGNet直接使用原始数据格式
    # 数据格式: (batch_size, 1024, 2) - 1024是序列长度，2是I/Q两个特征
    def prepare_data_for_csgnet(dataset):
        data = dataset.tensors[0]  # 已经是 (batch, seq_len, features) 格式
        labels = dataset.tensors[1]
        return data, labels
    
    valid_data, valid_labels = prepare_data_for_csgnet(valid_dataset)
    ...
    # 计算和显示总体性能
    print("\n" + "="*50)
    print("CSGNet OVERALL PERFORMANCE")
    print("="*50)
    ...
    overall_cm_fig = plot_confusion_matrix(
        overall_cm, 
        title="CSGNet Overall Confusion Matrix", 
    ...
    overall_cm_raw_fig = plot_confusion_matrix(
        overall_cm, 
        title="CSGNet Overall Confusion Matrix (Raw Counts)", 
        class_names=class_names,
        normalize=False
    )
    ...
    # 计算每个SNR下的性能
    print("\n" + "="*50)
    print("CSGNet PER-SNR PERFORMANCE")
    print("="*50)
    ...
    for snr in snr_list:
        ...
        if cm.sum() > 0:  # 确保该SNR有数据
            ...
            snr_cm_fig = plot_confusion_matrix(
                cm, 
                title=f"CSGNet Confusion Matrix (SNR = {snr} dB)", 
                class_names=class_names,
                normalize=True
            )
    ...
    with open(os.path.join(result_dir, 'classification_report.txt'), 'w') as f:
        f.write("CSGNet Classification Report\n")
        f.write("="*50 + "\n")
    ...
    print("CSGNet Test completed successfully!")
