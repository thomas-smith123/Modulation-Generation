...
class ProtoNet(nn.Module):
    ...
    def proto_train(self, sample):
        ...
        x_support = torch.from_numpy(x_support).to(next(self.encoder.parameters()).device)
        x_query = torch.from_numpy(x_query).to(next(self.encoder.parameters()).device)
        ...
        target_inds = target_inds.to(x_support.device)
