...
# @LastEditTime: 2025-08-18 16:30:42
# @FilePath: /complex_gru_single_radioml/main_real_only.py
# @Author: thomas-smith123 thomas-smith@live.cn
# @Description: 
# @
# @Copyright (c) 2024 by thomas-smith123 &&, All Rights Reserved. 
...
def train(rank, world_size, num_gpus, num_epochs, continue_train=False):
    ...
    if rank==0:
        # current_time = time.strftime("%Y%m%d-%H%M%S", time.localtime())
        tb_writer = SummaryWriter(os.path.join(PATH, current_time+'real_only'))
        tb_dir = os.path.join(PATH, current_time+'real_only')
    ...
    train_dataset.tensors[0][:,:,0] = torch.abs(train_dataset.tensors[0][:,:,0]**2+train_dataset.tensors[0][:,:,1]**2)
    train_dataset.tensors[0][:,:,1] = 0
    valid_dataset.tensors[0][:,:,0] = torch.abs(valid_dataset.tensors[0][:,:,0]**2+valid_dataset.tensors[0][:,:,1]**2)
    valid_dataset.tensors[0][:,:,1] = 0
    test_dataset.tensors[0][:,:,0] = torch.abs(test_dataset.tensors[0][:,:,0]**2+test_dataset.tensors[0][:,:,1]**2)
    test_dataset.tensors[0][:,:,1] = 0
