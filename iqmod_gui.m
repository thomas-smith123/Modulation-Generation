...
useHW = (isfield(handles, 'lastDownload') && strcmp(handles.lastDownload, 'HW'));
if (~recal && doCal)
...
if (result == 0 && doCal)
    ...
    if (isfield(handles, 'lastDownload') && strcmp(handles.lastDownload, 'DCA'))
        ...
        menuDCAVSA_Callback(hObject, eventdata, handles);
    elseif (isfield(handles, 'lastDownload') && strcmp(handles.lastDownload, 'M8135A'))
        pushbuttonDownload_Callback(hObject, eventdata, handles);
        menuUSPAVSA_Callback(hObject, eventdata, handles);
    else
        doLastDownload(hObject, eventdata, handles);
    end
    try
...
quadErr = iqparse(get(handles.editQuadErr, 'String'), 'vector');
iqskew = iqparse(get(handles.editIQSkew, 'String'), 'vector');
gainImbalance = iqparse(get(handles.editGainImbalance, 'String'), 'vector');
correction = get(handles.checkboxCorrection, 'Value');
multiCarrier = get(handles.checkboxMulti, 'Value');
...
if (div > 1 && length(find(sum(channelMapping, 2))) >= 4)
