...
%     
    ...
    % Don't allow downloads above 1GSa without the required option
    if size(data,1) > 2^30
        if (isfield(info, 'Options'))
    ...
    if (isfield(arbConfig, 'sampleMarker'))
        ...
        if (contains(arbConfig.sampleMarker, 'Once'))
            warndlg('Not supported by M8198A');
        ...
        elseif (contains(arbConfig.sampleMarker, '/'))
            ...
            switch div
                case 8 % special case (only supported with clock module)
                    xfprintf(f, sprintf(':OUTP:STAT "%s.RefClkOut16G",1', clkModuleID));
                case 16
                    xfprintf(f, sprintf(':OUTP:CLock:DIVider "%s.SampleMrk1",%.0f', arbConfig.M8070ModuleID, div));
                    xfprintf(f, sprintf(':OUTP:STAT "%s.SampleMrk1",1', arbConfig.M8070ModuleID));
                case 32
                    xfprintf(f, sprintf(':OUTP:CLock:DIVider "M2.SampleMrk1",%.0f', div));
                    xfprintf(f, sprintf(':OUTP:STAT "%s.SampleMrk1",1',arbConfig.M8070ModuleID));
                case 64
                    xfprintf(f, sprintf(':OUTP:CLock:DIVider "M2.SampleMrk1",%.0f', div));
                    xfprintf(f, sprintf(':OUTP:STAT "%s.SampleMrk1",1',arbConfig.M8070ModuleID));
                otherwise
                    warndlg({'Clock divide ratio not supported by M8198A - please change setting in Instrument configuration '});
