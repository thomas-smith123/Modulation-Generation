function f = iqdownload_M9484C(arbConfig, fs, data, marker1, marker2, segmNum, keepOpen, channelMapping, sequence, amplitude, fCenter, segmName, segmentLength, segmentOffset)
% Download a waveform to the M9484C
% It is NOT intended that this function be called directly, only via iqdownload
%
% B.Krueger, Keysight Technologies 2023-2024
%
...
% THIS SOFTWARE MAY ONLY BE USED IN CONJUNCTION WITH KEYSIGHT INSTRUMENTS. 
    ...
    byteOrderInstrument = xquery(f, ':SYST:WAV:BFIL:FORM:DEF?');
    if contains(upper(byteOrderInstrument), 'I16B')
        % Big endian
        if (strcmp(class(f), 'visa') || (strcmp(class(f), 'tcpip')))
            f.ByteOrder = 'bigEndian';
        end
        if ((strcmp(class(f), 'visadev.SOCKET')) || (strcmp(class(f), 'visalib.TCPIP')))
    ...
    if strcmp(arbConfig.model, 'M9484C_2Ch_COH')
        ...
        if ((isempty(amplitude) || isempty(fCenter) || isempty(segmName)) && segmentOffset == 0)
            fCenterSet1      = xquery(f, ':RF1:FREQuency?');
            amplitudeSet1    = xquery(f, ':RF1:POWer?');
            fCenterSet2      = xquery(f, ':RF2:FREQuency?');
            amplitudeSet2    = xquery(f, ':RF2:POWer?');
        ...
        if (isempty(segmentLength) || segmentOffset + length(data) >= segmentLength)
            doRun(f, segmNameSet, fs, fCenterSet1, amplitudeSet1, arbConfig.DACRange, 1, 1, segmNum);
            doRun(f, segmNameSet, fs, fCenterSet2, amplitudeSet2, arbConfig.DACRange, 2, 1, segmNum);
        end
        ...
        for channelIdx = 1:channelCount
            ...
            if any(channelMapping(channelIdx,:))
                ...
                % added a conditional for scripting
                if ((isempty(amplitude) || isempty(fCenter) || isempty(segmName)) && segmentOffset == 0)
                    fCenterSet1      = xquery(f, [':RF' channelString ':FREQuency?']);
                    amplitudeSet1    = xquery(f, [':RF' channelString ':POWer?']);
                ...
                % Only run the output if not doing a batch download
                if (isempty(segmentLength) || segmentOffset + length(data) >= segmentLength)
                    doRun(f, segmNameSet, fs, fCenterSet1, amplitudeSet1, arbConfig.DACRange, channelIdx, 1, segmNum);
                end
...
%         
%         % First adhere to the endianness of the instrument
%         % We can always set the endianness too, but this may be too
%         % invasive to the instrument control if it's different
%         byteOrderInstrument = xquery(deviceObject, ':SYST:WAV:BFIL:FORM:DEF?');
%         if contains(upper(byteOrderInstrument), 'I16B')
...
%         end  
        ...
        if numel(wave) > threshold
            ...
            if(segmentOffset > 0)
                xbinblockwrite(deviceObject,wave(1:threshold),'uint16',[':MEM:DATA:APPend "' ArbFileName '.wiq", ']);
            else
                xbinblockwrite(deviceObject,wave(1:threshold),'uint16',['MEM:DATA "NVWFM:' ArbFileName '", ']);
            ...
            while chunkEnd <= numSamples
                xbinblockwrite(deviceObject,wave(chunkIdx:chunkEnd),'uint16',[':MEM:DATA:APPend "' ArbFileName '.wiq", ']);
            ...
            xbinblockwrite(deviceObject,wave(chunkIdx:numSamples),'uint16',[':MEM:DATA:APPend "' ArbFileName '.wiq", ']);
%             xfprintf(deviceObject,'\n');
        else
            % Just send one portion of the waveform    
            if(segmentOffset > 0)
                xbinblockwrite(deviceObject,wave,'uint16',[':MEM:DATA:APPend "' ArbFileName '.wiq", ']);
            else
                xbinblockwrite(deviceObject,wave,'uint16',['MEM:DATA "NVWFM:' ArbFileName '", ']);
            end
        end
        ...
        if (~isempty(marker1) || ~isempty(marker2))
            ...
            if (~isempty(marker1))
                % Normalize
                marker1 = uint8(marker1/(max(marker1)));
            else
            ...
            if (~isempty(marker2))
                % Normalize
                marker2 = uint8(2*(~(marker2/(max(marker2)))));
            else
            ...
            if numMarkers > threshold
                ...
                % from last point we left off as the SCPI command set can only append
                if(segmentOffset > 0)
                    xbinblockwrite(deviceObject,marker(1:threshold),'uint8',[':MEM:DATA:APPend "' ArbFileName '.wmk", ']);
%                     xfprintf(deviceObject,'\n');
                else
                    % xfprintf(deviceObject, [':MMEMory:DELete "MKR1:' ArbFileName '"']);
                    xbinblockwrite(deviceObject,marker(1:threshold),'uint8',[':MEM:DATA "NVMKR:' ArbFileName '", ']);
%                     xfprintf(deviceObject,'\n');
                end
            ...
            while chunkEnd <= numMarkers
                xbinblockwrite(deviceObject,marker(chunkIdx:chunkEnd),'uint8',[':MEM:DATA:APPend "' ArbFileName '.wmk", ']);
                chunkIdx = chunkIdx + threshold + 1;
                chunkEnd = chunkIdx + threshold;
            ...
            % Write last portion
            xbinblockwrite(deviceObject,marker(chunkIdx:numMarkers),'uint8',[':MEM:DATA:APPend "' ArbFileName '.wmk", ']);
%             xfprintf(deviceObject,'\n');
            else
                % Just write the data once        
                if(segmentOffset > 0)
                    xbinblockwrite(deviceObject,marker,'uint8',[':MEM:DATA:APPend "' ArbFileName '.wmk", ']);
%                     xfprintf(deviceObject,'\n');
                else
                    xbinblockwrite(deviceObject,marker,'uint8',[':MEM:DATA "NVMKR:' ArbFileName '", ']);
%                     xfprintf(deviceObject,'\n');
                end
            end
...
function doRun(deviceObject, ArbFileName, sampleRate,...
    centerFrequency, outputPower, scalingFactor, runChannel, run, segNum)
...
%         opcComp = xquery(deviceObject, '*OPC?');
        xfprintf(deviceObject,[':SOURce:GROup' channelString ':SIGNal' num2str(segNum) ':MODE WAV']);
        xfprintf(deviceObject,[':SOURce:GROup' channelString ':SIGNal' num2str(segNum) ':WAVeform:SELect "NVWFM:' ArbFileName '"']);
        xfprintf(deviceObject,[':SOURce:GROup' channelString ':SIGNal' num2str(segNum) ':WAVeform:SELect "' ArbFileName '.wiq"']);
        opcComp = xquery(deviceObject, '*OPC?');
...
%         xfprintf(deviceObject,[':SIGNal' channelString ':WAV:SCAL ' num2str(scalingFactor)]);
        xfprintf(deviceObject,[':SOURce:GROup' channelString ':SIGNal' num2str(segNum) ':WAVeform:SCAL ' num2str(scalingFactor*100)]);
...
%         xfprintf(deviceObject,[':SIGNal' channelString ':WAV:SCL:RATE ' num2str(sampleRate)]);
        xfprintf(deviceObject,[':SOURce:GROup' channelString ':SIGNal' num2str(segNum) ':WAV:SCL:RATE ' num2str(sampleRate)]);
...
%         xfprintf(deviceObject,[':SIGNal' channelString ':STATe ON']) 
        xfprintf(deviceObject,[':SOURce:GROup' channelString ':SIGNal' num2str(segNum) ':STATe ON'])        
        % Modulation on
