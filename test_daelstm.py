...
Description: 改进的DAELSTM测试文件 - 已根据main_daelstm.py进行优化
- 支持生成不同SNR下的混淆矩阵和总体混淆矩阵
- 适配DAELSTM模型的数据格式 (batch, seq_len, features)
- 与main_daelstm.py保持一致的设置和优化
...
'''
# 导入需要的模块
from model_compare.daelstm import DAELSTM as Net  # 根据main.py使用的模型版本
import time
...
# 设置GPU - 与main_daelstm.py保持一致
os.environ["CUDA_VISIBLE_DEVICES"] = "0,7,1,2,3,4,5,6"
# 设置训练结果路径 - 与main_daelstm.py保持一致
PATH = './runs'
# 自动寻找最新的DAELSTM训练结果
if os.path.exists(PATH):
    daelstm_dirs = [d for d in os.listdir(PATH) if os.path.isdir(os.path.join(PATH, d)) and 'daelstm' in d]
    if daelstm_dirs:
        # 按时间排序，选择最新的
        daelstm_dirs.sort(reverse=True)
        PATH = os.path.join(PATH, daelstm_dirs[0])
        print(f"Using latest DAELSTM training results from: {PATH}")
    else:
        print(f"No DAELSTM training results found in {PATH}, using default path")
...
def validate_model(model, val_loader, device, class_num=15, snr_list=None):
    ...
    with torch.no_grad():
        for batch_idx, batch_data in enumerate(val_loader):
            ...
            # DAELSTM数据预处理：从复数转换为实数格式 (batch, seq_len, features)
            ...
            # 前向传播 - DAELSTM期望输入格式为 (batch, seq_len, features)
...
def test_model():
    ...
    # 准备验证数据加载器 - DAELSTM直接使用原始数据格式
    # 数据格式: (batch_size, 1024, 2) - 1024是序列长度，2是I/Q两个特征
    def prepare_data_for_daelstm(dataset):
        data = dataset.tensors[0]  # 已经是 (batch, seq_len, features) 格式
        labels = dataset.tensors[1]
        return data, labels
    
    valid_data, valid_labels = prepare_data_for_daelstm(valid_dataset)
