...
from model_compare.DLCNN import DLCNN as Net
import time
...
def train(rank, world_size, num_gpus, num_epochs, continue_train=False):
    ...
    if rank==0:
        # current_time = time.strftime("%Y%m%d-%H%M%S", time.localtime())
        tb_writer = SummaryWriter(os.path.join(PATH, current_time+'DLCNN'))
        tb_dir = os.path.join(PATH, current_time+'DLCNN')
        ...
        # 保存主要的Python文件
        try:
            shutil.copy2('main_DLCNN.py', os.path.join(tb_dir, 'main_DLCNN.py'))
            print(f"Saved main.py to {tb_dir}")
        except Exception as e:
            print(f"Failed to save main_DLCNN.py: {e}")
            
        # 保存网络模型文件
        try:
            shutil.copy2('model_compare/DLCNN.py', os.path.join(tb_dir, 'DLCNN.py'))
            print(f"Saved modelv0_7.py to {tb_dir}")
        except Exception as e:
            print(f"Failed to save DLCNN.py: {e}")
