        ...
        for batch_idx, batch_data in enumerate(val_loader):
            ...
            for i in range(len(predictions_np)):
                ...
                if current_snr in snr_confusion_matrices:
                    ...
                    # 添加边界检查，确保标签在有效范围内
                    if 0 <= true_label < class_num and 0 <= pred_label < class_num:
                        snr_confusion_matrices[current_snr][true_label][pred_label] += 1
                    else:
                        print(f"Warning: Invalid label detected - true_label: {true_label}, pred_label: {pred_label}, valid range: [0, {class_num-1}]")
...
def test_all_snr():
    ...
    snr_list = list(range(-20, 32, 2))
    print(f"Testing SNR range: {snr_list}")
    
    # 加载数据集
    print("Loading dataset...")
    train_dataset, train_snr, valid_dataset, valid_snr, test_dataset, test_snr = torch.load(
        "dataset/RadioML_dataset.pt", map_location='cpu', weights_only=False
    )
    
    # 从数据集中提取标签
    train_labels = train_dataset.tensors[1][:, 0].numpy()
    valid_labels = valid_dataset.tensors[1][:, 0].numpy()  
    test_labels = test_dataset.tensors[1][:, 0].numpy()
    ...
    # 如果标签已经是连续的0-based索引，直接使用
    if unique_labels.min() == 0 and len(unique_labels) == (unique_labels.max() + 1):
        print("Labels are already in valid 0-based continuous range")
        test_labels_mapped = test_labels
        actual_class_num = len(unique_labels)
        label_mapping = {i: i for i in range(actual_class_num)}
        reverse_mapping = {i: i for i in range(actual_class_num)}
    else:
        # 创建标签映射字典：原始标签 -> 连续索引 [0, num_classes-1]
        label_mapping = {orig_label: idx for idx, orig_label in enumerate(sorted(unique_labels))}
        reverse_mapping = {idx: orig_label for orig_label, idx in label_mapping.items()}
    ...
    # 创建显示用的类别名称（基于reverse_mapping）
    mapped_class_names = [f"Class_{reverse_mapping[i]}" for i in range(actual_class_num)]
    
    print(f"Using class_num = {actual_class_num}")
    
    # 创建带SNR信息的数据集（使用映射后的标签）
    # 注意：DatasetWithSNR 期望参数为 data, [labels, snr]
    val_dataset = DatasetWithSNR(test_dataset.tensors[0], [test_labels_mapped, test_snr])
    ...
    results_dir = os.path.join(PATH, f"test_results_CDSCNN_{timestamp}")
    ...
    all_predictions, all_labels, all_snrs, snr_confusion_matrices = validate_model(
        model, val_loader, device, actual_class_num, snr_list
    )
    ...
    plt.savefig(os.path.join(results_dir, 'cdscnn_overall_confusion_matrix.png'), dpi=300, bbox_inches='tight')
    plt.close()
    ...
    if snr_accuracies:
        ...
        plt.savefig(os.path.join(results_dir, 'cdscnn_snr_accuracy.png'), dpi=300, bbox_inches='tight')
        plt.close()
    ...
    for snr in key_snrs:
        if snr in snr_confusion_matrices and snr_confusion_matrices[snr].sum() > 0:
            ...
            plt.savefig(os.path.join(results_dir, f'cdscnn_confusion_matrix_snr_{snr}db.png'), dpi=300, bbox_inches='tight')
            plt.close()
    
    # 生成详细的分类报告（使用映射后的连续标签）
    print(f"\n=== Detailed Classification Report ===")
    # 创建显示用的类别名称（原始标签值）
    mapped_class_names = [f"Class_{reverse_mapping[i]}" for i in range(actual_class_num)]
    report = classification_report(all_labels, all_predictions, target_names=mapped_class_names, digits=4)
    print(report)
    print(f"\nLabel mapping used: {label_mapping}")
    
    # 保存结果到文件
    with open(os.path.join(results_dir, 'cdscnn_test_results.txt'), 'w') as f:
